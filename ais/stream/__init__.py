...
def normalize(nmea=sys.stdin,
    ...
    @param nmea: iterator of lines to read
    ...
    for idx, line in enumerate(nmea):
        try:
            ...
            if len(fields) < 6:
                errorcb(TooFewFieldsError(line_num=line_num, line=line.strip(), fields=len(fields)))
                continue
            ...
            if totNumSentences == sentenceNum:
                ...
                for part in parts:
                    ...
                    if ts1 > ts2+window or ts1 < ts2-window:
                        errorcb(DifferingTimestampsError(line_num=line_num, line=line.strip(), timestamp=timestamp, parts=parts))
                        ok=False
                        break
                if not ok: continue
